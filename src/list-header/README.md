# list-header 基础资料列表头部组件

### 引入

```js
import { kisListHeader } from 'kisUI';
```
...
```html
<kis-list-header :menulist="menulist" :searchObj="searchObj" />
```
...
### searchObj 输入组件参数 (组件默认数据)

```js
searchObj = {
  type: '', // 搜索框类别 （input可输入搜索、其他为不可输入搜索）
  text: '请输入商品名称/条码...', // 搜索框placeholder
  icon: 'scan', // 输入框里面的小图标
  keyword: '', // 搜索关键字
